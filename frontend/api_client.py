...
# (frontend/) as sys.path[0], which shadows the project-root `app` package.
...
def get_tasks() -> list[dict]:
    ...
    return _handle(requests.get(f"{BASE_URL}/tasks/", timeout=_TIMEOUT))


def get_all_tasks() -> list[dict]:
    """Fetch every task regardless of status."""
    return _handle(requests.get(f"{BASE_URL}/tasks/all", timeout=_TIMEOUT))


def get_completed_tasks() -> list[dict]:
    """Fetch tasks that have been marked complete."""
    return _handle(requests.get(f"{BASE_URL}/tasks/completed", timeout=_TIMEOUT))


def get_overdue_tasks() -> list[dict]:
    """Fetch tasks that are past their deadline."""
    return _handle(requests.get(f"{BASE_URL}/tasks/overdue", timeout=_TIMEOUT))


def create_task(payload: dict) -> dict:
    """Create a new task. Payload must match TaskCreate schema."""
    return _handle(requests.post(f"{BASE_URL}/tasks/", json=payload, timeout=_TIMEOUT))


def update_task(task_id: int, payload: dict) -> dict:
    """Partially update task fields."""
    return _handle(requests.patch(f"{BASE_URL}/tasks/{task_id}", json=payload, timeout=_TIMEOUT))


def complete_task(task_id: int) -> dict:
    """Mark a task as completed."""
    return _handle(requests.post(f"{BASE_URL}/tasks/{task_id}/complete", timeout=_TIMEOUT))


def delete_task(task_id: int) -> None:
    """Delete a task permanently."""
    _handle(requests.delete(f"{BASE_URL}/tasks/{task_id}", timeout=_TIMEOUT))
...
def chat_with_agent(message: str, available_hours: float) -> str:
    ...
    """
    payload = {"message": message, "available_hours": available_hours}
    data = _handle(requests.post(f"{BASE_URL}/agent/chat", json=payload, timeout=_TIMEOUT))
